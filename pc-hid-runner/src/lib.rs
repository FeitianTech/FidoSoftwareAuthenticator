...
use crate::gadget::{GadgetDevice, UsbGadget};
...
pub struct GadgetTransport<'pipe, 'interrupt> {
    _gadget: UsbGadget,
    device: GadgetDevice,
    host: CtaphidHost<'pipe, { DEFAULT_MESSAGE_SIZE }>,
    dispatch: ctaphid_dispatch::Dispatch<'pipe, 'interrupt, { DEFAULT_MESSAGE_SIZE }>,
...
impl<'pipe, 'interrupt> GadgetTransport<'pipe, 'interrupt> {
    pub fn new(
        gadget: UsbGadget,
        device: GadgetDevice,
        host: CtaphidHost<'pipe, { DEFAULT_MESSAGE_SIZE }>,
...
impl<'interrupt, D> Transport<'interrupt, D> for GadgetTransport<'_, 'interrupt>
where
    D: Dispatch,
{
    fn poll<A: Apps<'interrupt, D>>(&mut self, apps: &mut A) -> io::Result<bool> {
...
pub fn exec_gadget<'interrupt, D, A>(
    runner: Runner<D, A>,
    gadget: UsbGadget,
    platform: Platform,
    data: A::Data,
...
where
    ...
    A: Apps<'interrupt, D>,
{
    let device_path = gadget.device_path().to_path_buf();
    let device = GadgetDevice::open(&device_path)?;
    let channel: Channel<{ DEFAULT_MESSAGE_SIZE }> = Channel::new();
    let (requester, responder) = channel
        .split()
        .ok_or_else(|| io::Error::new(io::ErrorKind::Other, "failed to split CTAPHID channel"))?;
    ...
    host.set_capabilities(CAPABILITY_CBOR | CAPABILITY_NMSG);
    let dispatch = ctaphid_dispatch::Dispatch::new(responder);
    let transport = GadgetTransport::new(gadget, device, host, dispatch);
    runner.exec(platform, data, transport)
}
